...
var FilialaAction = function() {
    this.editedSpecIds = [];
    this.specTableSelection = null;
    this.table = "#filiala-table";
    this.getUrl = sunlightWorkspace.urlPrefix+"/config/app/getFilialaList";
    this.postUrl = sunlightWorkspace.urlPrefix+"/config/app/postFiliala";
}

FilialaAction.prototype.initPage = function() {
    this.getList();
}

FilialaAction.prototype.addNew = function(){
    var rowId = $.jgrid.randId();
    jQuery(this.table).addRowData( rowId, new FilialaObject("INSERT"));
    $(this.table).jqGrid('editRow', rowId,true);
}

FilialaAction.prototype.getList = function(){
...
FilialaAction.prototype.refreshList = function(){
    self = this;
    $.ajax({
        url: this.getUrl,
        type: "GET",
...
FilialaAction.prototype.afterGridLoad = function(){
}


FilialaAction.prototype.populateList = function() {
    self = this;
    $grid = $(this.table);
    $grid.jqGrid({
        url: this.getUrl,
        styleUI : 'Bootstrap',
        datatype: "json",
        colNames:['Id','Cod Filiala','Denumire','Data in', 'Data out', 'Save'],
...
FilialaAction.prototype.save = function(){
    ...
    for(var i=0;i <ids.length; i++) {
        ...
        if(_row.rowStatus == "INSERT"){
            console.log("filiala  "+JSON.stringify(_row, sunlightWorkspace.replaceSpaceString, ["id", "codFiliala","denumire","dataIn","dataOut"]));
            $.ajax({
                url: this.postUrl, type: 'POST',
                data: JSON.stringify(_row, sunlightWorkspace.replaceSpaceString, ["id", "codFiliala","denumire","dataIn","dataOut"]), dataType: 'json', contentType: "application/json",
                success: function (result) {
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    console.log('Error setting new Filiala');
                    console.log(jqXHR);
                }
            });
        }
